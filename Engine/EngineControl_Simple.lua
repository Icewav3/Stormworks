...
function onTick()
    ...
    DesiredRPS = input.getNumber(3)
    Charge = input.getNumber(4)
    ...
    if TargetRPS < minRPS then
        ...
        if (CurrentRPS == nil) or (CurrentRPS < minRPS) then
            starter = true
        else
            starter = false 
        end
    ...
    output.setBool(4,starter)
    output.setNumber(5,throttleout)
    output.setNumber(6,alternator)
    output.setBool(7,radiator)
end
