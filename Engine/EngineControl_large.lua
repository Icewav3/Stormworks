...
function onTick()
    ...
    if TargetRPS < minRPS then
        ...
        --clutch
        if GeneratorRPS > DesiredRPS then
            clutch = 0
        else
            if CurrentRPS < 3 then
                clutch = 0
            elseif CurrentRPS < (TargetRPS*0.1+TargetRPS) then
                clutch = clutch - 0.01
            else
                clutch = clutch + 0.1
            end
    ...
    output.setNumber(9,clutch)
    output.setBool(10,pumps)
end
